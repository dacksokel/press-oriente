...
$selector=mysqli_query($conectar, 'select titulo,contenido,autor,video,imagen,fecha,hora,posicion,id,publicar,urllink from noticiaseconomia')or die("error al conectarse a la tabla de economia ");
$respuestadar="";
while($sel=mysqli_fetch_array($selector)){
		...
		if($sel['publicar']=='si'&&$numerito==$sel['posicion']){
						...
						$respuestadar='							
						<a href="'.$sel['urllink'].'">
								<div class="imagenderecha" id="derechaeconomia">
									<img src="'.$sel['imagen'].'">
								</div>
