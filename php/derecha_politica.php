...
$selector=mysqli_query($conectar, 'select titulo,contenido,autor,video,imagen,fecha,hora,posicion,id,publicar,urllink from noticiaspolitica')or die("error al conectarse a la tabla de politica ");
$respuestadar="";
while($sel=mysqli_fetch_array($selector)){
		...
		if($sel['publicar']=='si'&&$numerito==$sel['posicion']){
						...
						$respuestadar='						
						<a href="'.$sel['urllink'].'">
								<div class="imagenderecha" id="derechapolitica">
									<img src="'.$sel['imagen'].'">
								</div>
