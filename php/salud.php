...
echo salud_parte1($conectar);
}
if($parte2==2){
echo salud_parte2($conectar);	
}
/*********************************************************************************************************
 * ************************************************************************************************************
 * ******************************************************************************************************/
function salud_parte1($conectar){
$arregloPrueba2=array();
$auxNoti="";
for($i=1;$i<=8;$i++){
	$auxNoti=$auxNoti.buscar_noticias_salud($conectar, $i);
...
function salud_parte2($conectar){
$arregloPrueba2=array();
$auxNoti="";
for($i=9;$i<=16;$i++){
	$auxNoti=$auxNoti.buscar_noticias_salud($conectar, $i);
...
function buscar_noticias_salud($conectar,$i){
	$res_aqui="";	
	$selector=mysql_query('select titulo,contenido,autor,video,imagen,fecha,hora,posicion,id,publicar,urllink from noticiassalud', $conectar)or die("error al conectarse a la tabla de salud ");
