...
echo zonapress_parte1($conectar);
}
if($parte2==2){
echo zonapress_parte2($conectar);	
}
/*********************************************************************************************************
 * ************************************************************************************************************
 * ******************************************************************************************************/
function zonapress_parte1($conectar){
$arregloPrueba2=array();
$auxNoti="";
for($i=1;$i<=8;$i++){
	$auxNoti=$auxNoti.buscar_noticias_zonapress($conectar, $i);
...
function zonapress_parte2($conectar){
$arregloPrueba2=array();
$auxNoti="";
for($i=9;$i<=16;$i++){
	$auxNoti=$auxNoti.buscar_noticias_zonapress($conectar, $i);
...
function buscar_noticias_zonapress($conectar,$i){
	$res_aqui="";	
	$selector=mysql_query('select titulo,contenido,autor,video,imagen,fecha,hora,posicion,id,publicar,urllink from noticiaszonapress', $conectar)or die("error al conectarse a la tabla de zonapress ");
